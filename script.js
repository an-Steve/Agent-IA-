  ...
  function type() {
    if (i < text.length) {
      if (text.substr(i, 4) === '<br>') {
        ...
        let closingIdx = text.indexOf('</a>', i);
        if (closingIdx !== -1) {
      ...
      setTimeout(type, Math.random() * 2 + 2); 
...
async function sendMessage() {
  ...
  if (lower.includes("meteo")) {
    ...
    showThinkingTrace("Appel de l'outil de navigation météo externe...");
    await sleep(800);
    hideThinkingTrace();
  ...
  // ── ROUTAGE ÉLARGI "GÉOGRAPHIE MONDIALE" & QUESTIONS DE GÉO -> GOOGLE MAPS ──
  if (
    lower.includes("geographie mondiale") || 
    lower.includes("google maps") || 
    lower.includes("carte du monde") || 
    lower.includes("geographie") ||
    lower.includes("capitale") || 
    lower.includes("sommet") || 
    lower.includes("pays")
  ) {
    chosenTool = "web_search";
    toolInput = { query: "google maps world" };
    toolOutput = "Routage cartographique : https://www.google.com/maps";
    finalResponse = "Pour explorer le monde, consulter des cartes détaillées et analyser la géographie mondiale, utilisez l'outil de cartographie :<br><br>👉 <a href='https://www.google.com/maps' target='_blank'>Cliquez ici pour ouvrir Google Maps</a>";
  ...
  // ── ANALYSE DES MATHÉMATIQUES ──
  const mathRegex = /([0-9]+[\s]*[\+\-\*\/][\s]*[0-9]+)/;
  if (mathRegex.test(text) || lower.includes("calcul") || lower.includes("combien font")) {
    chosenTool = "call_api";
    toolInput = { action: "math_computation", expression: text };
    
    showThinkingTrace("Calcul arithmétique en cours...");
    await sleep(900);
    
    try {
      const cleanExpr = text.replace(/[^0-9\+\-\*\/\(\)\.\s]/g, '');
      const result = eval(cleanExpr);
      toolOutput = `Calcul finalisé avec succès. Résultat = ${result}`;
      finalResponse = `L'analyse arithmétique donne le résultat suivant : **${result}**.`;
    } catch(e) {
      toolOutput = "Échec d'évaluation.";
      finalResponse = "Désolé, je n'ai pas pu résoudre cette expression.";
    }
    
    hideThinkingTrace();
    addToolCard(chosenTool, toolInput, toolOutput, 520);
    hideTyping();
  ...
  // ── DISCUSSIONS GÉNÉRALES PAR DÉFAUT ──
  showThinkingTrace("Modèle de langage en cours de traitement...");
  await sleep(1000);
  hideThinkingTrace();
  addToolCard(chosenTool, toolInput, toolOutput, 600);
  hideTyping();

  if (lower.includes("bonjour") || lower.includes("salut") || lower.includes("hello")) {
    finalResponse = getRandomResponse(AI_BRAIN.greetings_casual);
  } else if (lower.includes("comment va") || lower.includes("ca va") || lower.includes("status")) {
    finalResponse = getRandomResponse(AI_BRAIN.health_check);
  } else {
    finalResponse = "J'ai bien reçu votre demande : \"" + text + "\". Mes modules d'analyses autonomes sont à votre entière disposition.";
  }
